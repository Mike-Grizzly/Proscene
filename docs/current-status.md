...
**Session 2026-09-23 (long-book parser, round 4 — readable copies of unrenderable scans, automatic at upload):** the owner's remaining pain was that the scan libretto opened in the native PDF viewer (pdf.js draws 1-bit CCITT scans blank) so the Script tool was useless until a manual "Make searchable". Now every script-PDF entry point (`finalizeDocumentUpload`, `attachWizardScript`/`ByPath`, both `splitScriptDocument` halves) schedules `scheduleReadableCopy` → `after(installReadableCopy)`: detect a 1-bit scan (`readable-detect.ts`, pdf-lib XObject scan with a raw-bytes fallback), render every page with pdfium to a **1-bit PNG** (`encodePng` gained a 1-bit mode; ~23 KB and ~126 ms per page at 216 dpi) and assemble an image PDF with pdf-lib; the copy ("… (readable)") **becomes the default** when the original was, and members' preferences, the parse row and annotations are re-pointed to it; the original stays in Documents badged "Original scan". `documents.render_status` (`pending|done|skipped|failed`) drives a "Preparing a readable copy…" state on the Script tab and Focus that refreshes every 5 s. Documents page + `/productions/new` export `maxDuration = 300` and trace `pdfium.wasm`. "Make searchable" stays as the manual fallback. Migration `document_render_status` applied live. Tests: 1-bit PNG + detector (226 total). Not live-verified; see open-questions.

**Session 2026-09-23 (long-book parser, round 3 — structured outputs; searchable rebuild inherits the analysis):** live DB showed the in-app split and the scan libretto's parse **worked and applied** (20:42). The two later failures were on the "(searchable)" rebuild: (1) the rebuild is a new document, so it had no bookmarks; (2) its text-mode parse returned malformed JSON twice — OCR text is full of stray `"` characters and the prompt asks for verbatim anchors. Fixes: every model call now uses **structured outputs** (strict JSON schemas in `features/scripts/schemas.ts`, `output_config.format`, SDK 0.103 non-beta) with a one-shot repair call as fallback and honest "use Re-analyse" copy; `finalizeRebuiltScript` accepts `sourceDocumentId` (passed by the in-viewer "Make searchable") and **clones the applied analysis + seeds the AI bookmarks** onto the rebuild (`progress.clonedFrom`, excluded from quota), also re-pointing members' script preferences. `seedSharedBookmarks` moved to `features/scripts/bookmarks.ts`, `aiBookmarksFromResult` to `parse-utils.ts`. Tests: schema strictness + bookmark ids (222 total). Not live-verified; see open-questions.

**Session 2026-09-22 (follow-up — first live run of the long-book parser failed; pdfium raster fallback):** the owner re-ran the same 282-page scanned book after the merge and it failed again with a different message. Confirmed from the live DB: the new pipeline ran (mode `scan`, 5 chunks planned) and stopped because pdf-lib could not open the file, which the > 100-page path treated as fatal. **Reproduced locally with the real file (owner pushed it to `test-assets/beautiful-script`): pdf-lib opens it fine — the actual bug was that `unpdf`/pdf.js detaches the ArrayBuffer it is handed, so pdf-lib (and, since June, the scan fingerprint — `sha256("")` for every scan) read an empty buffer.** Fix 1: text extraction moved to `pdf-text.ts` and works on a copy (regression test). Fix 2 (belt and braces): the scan path no longer depends on pdf-lib parsing — when it fails, **pdfium** (already a dependency, run from its Node build server-side) renders pages to grayscale PNGs (own zlib-only encoder, `png-encode.ts`) and the chunks are re-planned at ≤ 24 pages and sent as image blocks (`progress.engine = "pdfium-raster"`); detection samples and the split use the same fallback (split halves become image-only PDFs, flagged `rasterized`). `pdfium.wasm` is traced into the run route + the two split pages via `outputFileTracingIncludes`; `@hyzyla/pdfium` added to `serverExternalPackages`. The pdf-lib error is now stored on the row (`progress.pdfLibError`) and surfaced in the failure text. New tests for the PNG encoder (214 total). The `test-assets/beautiful-script` branch holds the owner's copy of the file for reproduction and should be deleted once verified (licensed script; never merge it). Local end-to-end: pdf-lib cuts all five 60-page chunks (~1.7 MB each) and pdfium opens the file too. **Not yet live-verified.**

**Session 2026-09-22 (long-book AI parsing — chunked/resumable runs, libretto + vocal-score split, per-member script switching; branch `claude/loving-babbage-ggkydb`, NOT live-verified):** the owner's combined libretto + piano-vocal score PDF was refused by the AI parse ("too many pages"). Root causes: scans were hard-rejected above `MAX_SCANNED_PAGES = 250`, text PDFs were **silently truncated** at 600k chars, and the whole book went to Claude in one call inside one 300-s `after()` invocation. (1) **Chunked, resumable parse** — `runScriptParse` now plans page-range chunks (text ≤ 450k chars per chunk, so ordinary scripts stay single-call; scans ≤ 60 pages / 18 MB per chunk cut with **pdf-lib**, new dep approved), runs them **sequentially with carry-forward context** (roles so far, last scene) and merges (`mergeChunkResults`); a **DB lease + heartbeat** on `script_parses` (`lease_token`, `lease_expires_at`, `progress`, `page_count`) lets the run route hand off across invocations — the worker yields at ~90 s, releases the lease and **self-kicks** `/run` with the `CRON_SECRET` bearer convention; the review page's poll re-kicks when the row looks `resumable`. Hard cap `MAX_SCRIPT_PAGES = 600`. Staleness watchdog now keys on the last heartbeat (`updated_at`). Progress shown as "Analysing pages X–Y of Z". (2) **Libretto / vocal-score detection + split** — text heuristics (`classifyTextPage`/`deriveSections`, 12-page island smoothing) with a cheap model confirmation only when mixed; scans via a sampled sub-PDF + boundary refinement. A mixed book stops at new status **`split_suggested`** with an editable two-range proposal; **"Split & continue"** (`splitScriptDocument`) cuts two `documents` rows (`script_kind` libretto / vocal_score, `source_document_id`, page range, `page_count`), keeps the original as "combined", makes the libretto the default and stages its analysis; **"Analyse as one book"** continues. Score parses use a score preface and **apply add-only** (roles `source = "ai_score"`, no scene changes, song bookmarks on the score doc). (3) **Per-member script switching** — new table **`script_preferences`** (user, production, active_script_id); `getActiveScript` = own choice else production default; `ScriptSwitcher` in the viewer toolbar / Focus / phone header (`setMyActiveScript`), managers "Make default" (`setProductionDefaultScript`). Switching **never** bumps `scriptVersion` or flags annotations stale (that stays with the Documents-tab "Set as default script"); the viewer is keyed by script id so a switch remounts. Documents tab shows a kind badge; the review page has a per-script picker with "Analyse". **Schema:** migration `script_split_and_chunked_parse` applied live via Supabase MCP (documents +5 cols, script_parses +4 cols, `script_preferences` table, RLS on / no policies); Drizzle synced. **Tests:** 26 new vitest cases for the pure helpers (`parse-utils.test.ts`) — 211 total passing; `tsc` clean; `eslint` 0 errors; `next build` green. **Branch audit (owner ask):** every merged PR is on `main`; the only finished unmerged branch is the 2026-07-22 security review — surfaced as **PR #86** for the owner to merge; the other unmerged branches (tours, Eos prototype, Builder.io POC, open PRs #48/#64) are unverified/prototype/conflicted and were left alone. Spec: `feature-specs/19-ai-script-analysis.md` → "Long books". Decisions: `decision-log.md` (2026-09-22). Follow-ups: `open-questions.md` (2026-09-22).
